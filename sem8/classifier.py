...
SIZE = 256  # Change this size according to your model's expected input size
...
class MainModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 16 * (SIZE // 2) * (SIZE // 2))  # Flatten the tensor
        x = self.fc1(x)
        return x
...
def load_model(model_path):
    model = MainModel()  # Initialize the model structure
...
def classify_image(model, image_path):
    image_tensor = preprocess_image(image_path)  # Preprocess the image
    with torch.no_grad():  # No need to compute gradients
        output = model(image_tensor)  # Forward pass
    predicted_label = output.argmax(dim=1).item()  # Get the predicted class index
    return predicted_label

# Load your model
model_path = 'model_weights.pth'  # Replace with your model's path
...
if not os.path.exists(image_path):
    ...
    # Map the predicted class index to category name
